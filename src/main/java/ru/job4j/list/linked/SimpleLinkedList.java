...
public class SimpleLinkedList<E> implements List<E> {
    ...
    /**
     * Данный метод переворачивает
     * связанный список.
     *
     * Пример для 3 узлов.
     * 1.Проверяем, что список пуст
     * или в нем один элемент.
     *
     * Начинаем перелинковку.
     * 2.Присваиваем текущему узлу
     * следующий узел (1 = 2), который в
     * себе содержит значение и ссылку на
     * следующий элемент (3).
     *
     * 3.У head-узла обнуляем
     * ссылку на следующий узел.
     *
     * 4.В цикле "пока текущий узел
     * не равен null" проводим
     * многоходовочку.
     *
     * 5.Объявляем узел {@code next}
     * и присваиваем ему ссылку
     * на следующий узел (3) текущего узла (2).
     *
     * 6.Ранее в п.3 мы обнулили ссылку
     * текущего узла (это head-узел).
     * Теперь мы говорим, что следующим
     * узлом текущего узла (2) будет
     * head-узел (1).
     *
     * 7.После этого head-узлом
     * становится текущий узел.
     * То есть узел (2) == head-узел.
     *
     * 8.Текущи узел становится
     * next-узлом.
     *
     * Таким образом, {@code currentEl}
     * является указателем, который
     * перескакивает от узла к узлу
     * и меняет там ссылки.
     */
    public boolean revert() {
        ...
        Node<E> currentEl = firstNode.next;
        firstNode.next = null;
        while (currentEl != null) {
            Node<E> next = currentEl.next;
            currentEl.next = firstNode;
            firstNode = currentEl;
            currentEl = next;
        }
        return true;
